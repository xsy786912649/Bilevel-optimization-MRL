...
if os.path.exists("./check_point/running_state_Ant_vel_"+str(index)+".pkl"):
    with open("./check_point/running_state_Ant_vel_"+str(index)+".pkl",'rb') as file:
...
if __name__ == "__main__":
    if not os.path.exists("./check_point/meta_policy_net_Ant_vel_"+str(index)+".pkl"):
        meta_policy_net = Policy(num_inputs, num_actions)
    else:
        meta_policy_net = torch.load("./check_point/meta_policy_net_Ant_vel_"+str(index)+".pkl")
    ...
    for i_episode in range(500):
        ...
        with open('./check_point/training_log_Ant_vel_'+str(index)+'.csv', 'a+') as file:
        ...
        if result_after.mean()>aaaaaa:
            ...
            torch.save(meta_policy_net, "./check_point/meta_policy_net_Ant_vel_"+str(index)+".pkl")
            output_hal = open("./check_point/running_state_Ant_vel_"+str(index)+".pkl", 'wb')
            str1 = pickle.dumps(running_state)
            output_hal.write(str1)
            output_hal.close()
        
        #torch.save(meta_policy_net, "./check_point/meta_policy_net_Ant_vel_"+str(i_episode)+".pkl")
        #output_hal = open("./check_point/running_state_Ant_vel_"+str(i_episode)+".pkl", 'wb')
        #str1 = pickle.dumps(running_state)
        #output_hal.write(str1)
        #output_hal.close()

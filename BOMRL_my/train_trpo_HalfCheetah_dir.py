...
parser.add_argument('--max-kl', type=float, default=3e-2, metavar='G',    #3e-3 is good for one time
...
if os.path.exists("./check_point/running_state_HalfCheetah_dir_"+str(index)+".pkl"):
...
if __name__ == "__main__":
    if not os.path.exists("./check_point/meta_policy_net_HalfCheetah_dir_"+str(index)+".pkl"):
    ...
    for i_episode in range(500):
        ...
        with open('./check_point/training_log_HalfCheetah_dir_'+str(index)+'.csv', 'a+') as file:
        ...
        if result_after.mean()>aaaaaa:
            ...
            torch.save(meta_policy_net, "./check_point/meta_policy_net_HalfCheetah_dir_"+str(index)+".pkl")
            output_hal = open("./check_point/running_state_HalfCheetah_dir_"+str(index)+".pkl", 'wb')
            str1 = pickle.dumps(running_state)
            output_hal.write(str1)
            output_hal.close()
        
        #torch.save(meta_policy_net, "./check_point/meta_policy_net_HalfCheetah_dir_"+str(i_episode)+".pkl")
        #output_hal = open("./check_point/running_state_HalfCheetah_dir_"+str(i_episode)+".pkl", 'wb')
        #str1 = pickle.dumps(running_state)
        #output_hal.write(str1)
        #output_hal.close()
